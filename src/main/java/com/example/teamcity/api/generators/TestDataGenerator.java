...
public final class TestDataGenerator {
    ...
    public static TestData generate() {
        ...
        try {
            ...
            // Dynamically creates a new object of the given class using reflection
            // Equivalent to 'new TestData()', but works even when the class type is only known at runtime

            // Reflection lets Java inspect classes and create objects dynamically at runtime

            var generatedModels = new ArrayList<BaseModel>(); // накапливаем

            for (var field : TestData.class.getDeclaredFields()) {
                field.setAccessible(true); // can appeal to private field
            ...
            return instance;
                 ...
                 NoSuchMethodException e) {
            throw new IllegalStateException("Cannot generate tests data", e);
        }
    }
